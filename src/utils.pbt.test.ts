...
// オブジェクトからURLSearchParamsを作成するヘルパー関数
...
describe('parseQueryString プロパティベーステスト', () => {
  // 異なるパラメータタイプのArbitraries
  ...
  test('parseQueryStringは有効な入力に対して例外をスローしないこと', () => {
    fc.assert(
  ...
  test('parseQueryStringは空の入力に対して空のオブジェクトを返すこと', () => {
    fc.assert(
  ...
  test('parseQueryStringは文字列パラメータを正しく解析すること', () => {
    fc.assert(
  ...
  test('parseQueryStringは真偽値パラメータを正しく解析すること', () => {
    fc.assert(
  ...
  test('parseQueryStringは真偽値を表す文字列を正しく解析すること', () => {
    fc.assert(
      fc.property(fc.boolean(), fc.boolean(), (audio, video) => {
        // 真偽値を文字列に変換
        const params = {
  ...
  test('parseQueryStringは指定された文字列パラメータを正しく解析すること', () => {
    fc.assert(
      fc.property(
        audioCodecTypeArb,
        roleArb,
  ...
  test('parseQueryStringはsignalingUrlCandidatesを正しく解析すること', () => {
    fc.assert(
  ...
  test('parseQueryStringは無効なJSONのsignalingUrlCandidatesを処理すること', () => {
    fc.assert(
      fc.property(
        fc.string().filter((s) => {
          try {
  ...
  test('parseQueryStringはundefinedのプロパティを削除すること', () => {
    fc.assert(
      fc.property(
        fc.string(),
        fc.string().filter((s) => !AUDIO_CODEC_TYPES.includes(s as '' | 'OPUS')),
  ...
  test('parseQueryStringは解像度の形式を正しく処理すること', () => {
    fc.assert(
      fc.property(
  ...
  test('parseQueryStringは有効な入力に対して冪等であること', () => {
    fc.assert(
      fc.property(parametersArb, (params) => {
        const searchParams = createSearchParams(params)
        const result1 = parseQueryString(searchParams)

        // 結果から新しいsearchParamsを作成して再度解析
  ...
  test('parseQueryStringはすべてのパラメータタイプの混合を処理すること', () => {
    fc.assert(
      fc.property(
        fc.string(),
        fc.boolean(),
