...
実装時に行番号がずれている可能性があるため、コンポーネント名（`MediaTypeForm` / `FormRadio`）および関数名（`setMediaType` / `checkFormValue`）を基準に特定すること。Polished 時点は 2026-06-09。

### `FormCheck` の `<input>` に `value` 属性が無い（真の根本原因）

`src/components/ui/FormCheck.tsx:69-94` の `<input type="radio">` / `<input type="checkbox">` は `value` props を受け取らず `<input>` にも `value` 属性を伝搬しない。HTML Living Standard §4.10.5.1.16 で `<input>` の `value` IDL 属性は未指定時 `"on"` を返すと規定されており、Chromium / Gecko / WebKit すべてでこの挙動。よって `event.target.value` を読むと常に `"on"` が返り、`MEDIA_TYPES` (`"getUserMedia" | "getDisplayMedia" | "fakeMedia" | "mp4Media"`) のいずれともマッチしない。

### `FormRadio` の合成 Event 偽装

`src/components/DevtoolsPane/MediaTypeForm.tsx:14-39` の `FormRadio`:
...
`new Event("change")` を生成 → `Object.defineProperty` で偽の `target.value` を貼る → 親の `onChange` に渡す。DOM Standard §2.7 で `Event.target` の `[[Configurable]]` は true のためどのブラウザでも上書きは成功する（issue 旧版が主張した「Firefox / Safari で `undefined`」は誤認）。

### `MediaTypeForm` 親側の `onChange`

`src/components/DevtoolsPane/MediaTypeForm.tsx:45-49`:
...
`SoraDevtoolsState["mediaType"]` は `(typeof MEDIA_TYPES)[number]` に解決される（`src/types.ts` で定義）。
...
### 1. `FormRadioProps` の型変更

`src/components/DevtoolsPane/MediaTypeForm.tsx:14-19`:
...
### 2. `FormRadio` 内部 `onChange` の書き換え

`src/components/DevtoolsPane/MediaTypeForm.tsx:28-35`:
...
### 3. `MediaTypeForm.onChange` の置き換え + `checkFormValue` 削除
...
`MediaTypeForm.tsx:78-85` の `mountClient.value && <FormRadio label="mp4Media" ... disabled={disabled || !isMp4MediaAvailable} ... />` は `"mp4Media"` が `MEDIA_TYPES` (`src/constants.ts:67`) に含まれている前提で新型に適合する。`mountClient` / `isMp4MediaAvailable` ガード自体は本 issue では触らない（SSR / CSR 不一致回避のため必要）。
...
`CHANGES.md` の `## develop` の `### misc` セクション末尾に以下を追記する。担当者行を忘れないこと。

既存 `### misc` エントリは `[CHANGE]` / `[ADD]` / `[UPDATE]` の種別タグを付ける慣習に従う（CLAUDE.md「各エントリは `- [種別] 変更内容を〜するという形で書く`」）。本修正は内部実装の置き換えで挙動変更を伴わないが、`### misc` 内の前例（Preact / Tailwind / Vite+ 移行など内部実装の変更）に倣って `[CHANGE]` を付ける。
...
- **`FormCheck` への `value` props 追加案（不採用）**: `FormCheck` (`src/components/ui/FormCheck.tsx`) に `value?: string` props を追加し `<input value={value}>` で伝搬すれば、`MediaTypeForm` 側は `event.target.value` をそのまま読めて合成 Event が不要になる。ただし:
  ...
  以上から本 issue では採用せず、`MediaTypeForm.tsx` 内 `FormRadio` 局所で `(value: (typeof MEDIA_TYPES)[number]) => void` シグネチャに統一する。`FormCheck` の API 拡張が必要になった時点で別 issue として SEQUENCE を取得し起票する。
...
5. `?mediaType=fakeMedia` などの URL クエリ反映 (`src/app/actions.ts:193` の `setMediaType` 経路) が壊れていないことを確認する。
...
9. `vp test` が pass すること（`MEDIA_TYPES` を扱う既存テストの非退行）。
10. 既存 Playwright e2e が pass すること。
...
- `CHANGES.md` の `## develop` の `### misc` セクション末尾に「6. CHANGES.md エントリ」の文面が追記され、担当者行が付いていること。
