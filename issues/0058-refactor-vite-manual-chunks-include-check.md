...
行番号は陳腐化するため記載しない。各箇所はファイル名（`vite.config.ts`）と関数名（`manualChunks`）で特定する。
...
本リポジトリは pnpm を使用。pnpm では `node_modules/preact` は `.pnpm/preact@<version>/node_modules/preact/` への symlink で、Rolldown / Vite が `manualChunks` に渡す `moduleId` は resolve 後の実パスになる:

- npm / yarn: `/abs/path/node_modules/preact/dist/preact.module.js`
- pnpm: `/abs/path/node_modules/.pnpm/preact@<version>/node_modules/preact/dist/preact.module.js`
...
output: {
  manualChunks(moduleId) {
    ...
    // pnpm の場合 moduleId は `.../node_modules/.pnpm/preact@<version>/node_modules/preact/dist/preact.module.js`
...
- **e2e 追加なし**: 既存 `tests/mp4-media-stream-lazy-load.test.ts` / `tests/noise-suppression-lazy-load.test.ts` は「動的 import がネットワークを介して遅延ロードされる」ことを URL 文字列の正規表現で確認するもので、chunk 分割の境界そのものは検証していない。本 issue の予防的修正でも既存テストの動作は変わらず、追加 e2e も不要（chunk 分割の本質確認は検証手順 B の手動検証で行う）。
- **手動検証**: 後述「検証手順」で `pnpm build` 後の `dist/assets/*.js` のファイル名一覧と、`preact-render-to-string` 一時追加時の chunk 分布を確認する。
...
`CHANGES.md` の `## develop` 内 `### misc` セクション末尾に以下を追記する。担当者行を忘れないこと。既存 `### misc` エントリの種別タグ慣習に従い `[CHANGE]` を付ける（内部実装の変更扱い）。
...
- 検証手順 A-D すべてが通過すること。
- 修正後コード（設計方針 after）と一致して実装されていること（`includes("/node_modules/${mod}/")` の末尾 `/` 付き境界一致になっていること）。
- `CHANGES.md` の `## develop` の `### misc` セクション末尾に上記エントリが追記され、担当者行が付いていること。
- 既存テスト（`pnpm test`）および既存 Playwright e2e（`pnpm test:e2e`）が pass すること。
