...
- ユーザーの最新の設定を最終的な MediaStream と Sora の送信状態へ反映する。処理を破棄する場合は、生成済みのトラックと `AudioContext` を確実に解放し、Fake Video Worker の描画と Media Processor の処理を確実に停止する
...
- 競合で破棄された MediaStreamTrack と `AudioContext` が残留せず、Fake Video Worker の描画と Media Processor の処理が継続しない
...
- 古い処理を破棄する場合は、処理中に生成した MediaStreamTrack と `AudioContext` を解放し、Fake Video Worker の描画と Media Processor の処理を停止する
