...
1. `package.json` の `dependencies` に `preact-iso` を追加する
   ...
   - 現状の最新版 (`2.12.0`) は十分にリリースから時間が経っており `minimumReleaseAge` の制約に引っかからない。`pnpm-workspace.yaml` の `minimumReleaseAgeExclude` への追記は不要
...
7. `src/components/Header/CopyUrlButton.tsx` を修正し、`copyURL()` 実行後に `useLocation().route(globalThis.location.pathname + globalThis.location.search, true)` を呼び出す
...
9. `src/DevTools.tsx` の cleanup から `disconnectSora()` を削除し、`beforeunload` イベントリスナーで `disconnectSora()` を呼ぶように変更する
   - `DevTools` マウント時に `addEventListener('beforeunload', handleBeforeUnload)`、アンマウント時に `removeEventListener` する
   - `handleBeforeUnload` 内では `void disconnectSora()` を呼び出す
   - `beforeunload` 内での非同期完了は保証できないため、ブラウザ終了時は切断完了を保証できない
10. Playwright E2E テストで `/` と `/sessions` の遷移を検証する
...
- `beforeunload` イベント内での `disconnectSora()` の非同期完了は保証できないが、これは #0067 の「切断不確定」表示方針で対応する
